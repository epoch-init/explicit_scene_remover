...
def export_clean_media(video_path, srt_path, cuts, output_dir, mode="fast"):
    ...
    Maintains the original file extension (e.g., .mp4 -> _clean.mp4, .avi -> _clean.avi)
    """
    os.makedirs(output_dir, exist_ok=True)
    
    base_name = os.path.splitext(os.path.basename(video_path))[0]
    ext = os.path.splitext(video_path)[1].lower() # .mp4 or .avi
    
    out_video_path = os.path.join(output_dir, f"{base_name}_clean{ext}")
